"""Export the public record of the v3.5 GPT-5.5 versus GPT-5.6 Luna comparison.
...
host paths. Run from the site root:
    ...
    python3 scripts/export_swe_v4_v35_evidence.py --harness-root ../VulcanBench
"""
...
DEST = SITE / "assets/data/swe-v4-gpt55-luna-v35"
...
RATES_PER_MILLION = {"gpt55": {"input": 5.00, "cached_input": 0.50, "output": 30.00},
...
def main():  # noqa: PLR0915, one linear export
    ...
    v35 = root / "runs-code-quality-maintenance-v3.5"
    summary = read(v35 / "summary.json")
    manifest = {r["id"]: r for r in read(v35 / "private-manifest.json")}
    protocol = read(v35 / "protocol.json")
    comparison_path = root / "docs/results/swe-v4-gpt55-luna-2026-09/comparison.json"
    comparison = read(comparison_path)
    assert summary["ready_for_publication"] and summary["published_submissions"] == 207
    ...
    for entry in summary["rows"]:
        ...
        assert abs(priced(receipt, entry["model"]) - run["api_equivalent_cost_usd"]) < 1e-5, run["run_id"]
        panels = entry["panels"]
        ...
        rows.append({
            ...
            "estimated_usd": run["api_equivalent_cost_usd"],
    ...
    with (SITE / "assets/data/swe-v4-gpt55-luna-v35-scores.csv").open("w", newline="") as handle:
        writer = csv.writer(handle, lineterminator="\n")
    ...
    totals = {m: {"runs": sum(1 for r in rows if r["model"] == m),
              ...
              for m in LEVELS}
    save("economics.json", {
        "scope": "Solver inference only, API-equivalent at list rates; both models ran on a ChatGPT subscription through Codex, "
        ...
        "limitations": [
            ...
            "One GPT-5.5 extra-high run (paddockcore) was re-run after the first attempt overran its cap under a harness fault; "
            "the re-run is the priced and judged run and the capped attempt is excluded.",
        ],
    ...
    save("provenance.json", {
        "source_artifacts_sha256": {
            "v3.5/summary.json": digest(v35 / "summary.json"), "v3.5/protocol.json": digest(v35 / "protocol.json"),
            "v3.5/private-manifest.json": digest(v35 / "private-manifest.json"),
            "v3.5/calibration-muse.json": digest(v35 / "calibration-muse.json"), "v3.5/calibration-grok.json": digest(v35 / "calibration-grok.json"),
            "comparison.json": digest(comparison_path),
        },
        "export_checks": ["207 published submissions across nine model and effort cells, none excluded", "both scored panels passed calibration under v3.5",
                          ...
                          "every run's API-equivalent cost recomputed from its Codex receipt at the published list rates",
