...
(function () {
  ...
  function chart() {
    ...
    for (var y = ymin; y <= ymax; y += 10) {
      g += '<line x1="' + L + '" y1="' + Y(y).toFixed(1) + '" x2="' + (W - R) + '" y2="' + Y(y).toFixed(1) + '" stroke="#1c1a17" stroke-opacity="0.09"/>';
      g += '<text x="' + (L - 10) + '" y="' + (Y(y) + 4).toFixed(1) + '" text-anchor="end" font-size="12" fill="#6f6a62">' + y + "</text>";
    }
    for (var t = 0; t <= xmax + 1e-9; t += step) {
      g += '<line x1="' + X(t).toFixed(1) + '" y1="' + (H - B) + '" x2="' + X(t).toFixed(1) + '" y2="' + (H - B + 5) + '" stroke="#1c1a17" stroke-opacity="0.4"/>';
      g += '<text x="' + X(t).toFixed(1) + '" y="' + (H - B + 20) + '" text-anchor="middle" font-size="12" fill="#6f6a62">' + ax.fmt(t) + "</text>";
    }
    g += '<line x1="' + L + '" y1="' + (H - B) + '" x2="' + (W - R) + '" y2="' + (H - B) + '" stroke="#1c1a17" stroke-opacity="0.4"/>';
    g += '<text x="' + ((L + W - R) / 2).toFixed(1) + '" y="' + (H - 10) + '" text-anchor="middle" font-size="12.5" fill="#3a362f">' + esc(ax.label) + "</text>";
    g += '<text transform="translate(16 ' + ((T + H - B) / 2).toFixed(1) + ') rotate(-90)" text-anchor="middle" font-size="12.5" fill="#3a362f">VulcanBench Frontier v4 score</text>';
    var labels = [];
    MODELS.forEach(function (m) {
      ...
      if (pts.length > 1) g += '<polyline fill="none" stroke="' + col + '" stroke-width="2" stroke-linejoin="round" points="' + pts.map(function (r) { return X(r[xKey]).toFixed(1) + "," + Y(r.combined).toFixed(1); }).join(" ") + '"/>';
      pts.forEach(function (r) {
        var tip = r.model + " " + LABEL[r.effort] + ": " + r.combined.toFixed(1) + ", " + ax.tip(r);
        g += '<circle cx="' + X(r[xKey]).toFixed(1) + '" cy="' + Y(r.combined).toFixed(1) + '" r="5.5" fill="' + col + '" stroke="#f7f4ee" stroke-width="1.5"><title>' + esc(tip) + "</title></circle>";
      });
      var last = pts.reduce(function (a, b) { return b[xKey] > a[xKey] ? b : a; });
      labels.push({ x: X(last[xKey]) + 11, y: Y(last.combined) + 4, text: m.name, col: col });
    });
    ...
    return '<svg viewBox="0 0 ' + W + " " + H + '" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="VulcanBench Frontier v4 score against ' + esc(ax.label.toLowerCase()) + ', one line per model across its effort levels" font-family="IBM Plex Mono, SF Mono, monospace">' + g + "</svg>";
  }
  ...
  function render() {
    ...
    app.innerHTML = '<div class="v4group"><span class="v4label">X axis</span><div class="lb-toggle chart-toggle on v4modes" role="tablist" aria-label="Chart x axis">' + tabs + "</div></div>" +
      '<figure class="v4card v4chart">' + chart() + '<figcaption><span>Each line is one model through its reasoning-effort levels, Low on the left to Max on the right. Hover a point for the exact numbers.</span></figcaption></figure>';
