/* VulcanBench: share benchmark tables and model cards as images. Zero dependencies. */
(function () {
  "use strict";
  ...
  function shareCanvas(canvas, filename) {
    canvas.toBlob(function (blob) {
      if (!blob) return;
      shareBlob(blob, filename, pageTitle() + " | VulcanBench");
    }, "image/png");
  }
  ...
  function init() {
    ...
    tables.forEach(function (table, i) {
      ...
      btn.innerHTML = SHARE_ICON + '<span>Share table</span>';
      btn.addEventListener("click", function () {
        var suffix = tables.length > 1 ? "-t" + (i + 1) : "";
        shareCanvas(renderCard(table), "vulcanbench-" + slug + suffix + ".png");
      });
    ...
    figures.forEach(function (figure) {
      ...
      var wrap = document.createElement("div");
      wrap.className = "card-share-wrap";
      var btn = document.createElement("button");
      btn.className = "table-share-btn";
      btn.type = "button";
