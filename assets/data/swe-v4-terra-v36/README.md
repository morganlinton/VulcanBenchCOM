...
Public record for the September 16, 2026 GPT-5.6 Terra report: 114 solver
...
complexity, and security measurements come from the sweep. The Max cell holds
22 runs: paddockcore never started because the subscription's quota window
closed until September 19, 2026, and it will be judged as a separate top-up
freeze; this record is not rewritten.
...
| [runs.csv](runs.csv) | The same 114 records flat for spreadsheets |
...
| [judge-protocols.json](judge-protocols.json) | The exact system text, rubric, pair instruction, probe and match instructions, schemas, weights, repeats, seed, allowance rule, control source hashes, the population record and the missing run as frozen |
...
score; this applies to one Low run (granarycore) and one High run (schedcore). The group
...
Each judge made 437 calls: 80 in calibration, 114 primary reviews, 5 repeats,
10 pairwise checks, 114 intent probes and 114 answer-key matches. Muse needed
...
each retried under the network-fault rule. Neither judge produced a reviewer
fallback. Every attempt is archived in the harness run directory.

## Population note

Paddockcore at Max has no attempt. From 06:00 PDT on September 15, 2026
every launch was refused by the Codex API with "You've hit your usage limit
... try again at Sep 19th, 2026 1:10 AM" before any work; one refused attempt
is kept in the harness as evidence. The cell froze at 22 on the owner's
decision to publish now and top up later; the top-up is a separate v3.6.1
freeze under the same calibration and does not rewrite this record. No run
was excluded.
